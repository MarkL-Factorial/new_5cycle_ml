...
Returns a flat dict so it composes cleanly into per-seed rows. Per-cohort AUC
is NaN when a slice has <2 samples or only one class.
"""
...
def metrics_from_predictions(
    ...
    """Compute all five metrics + per-cohort AUC from pre-computed predictions."""
    out: dict[str, Any] = {
        "n": int(len(y)),
...
def evaluate(
    ...
    return metrics_from_predictions(y, y_pred, y_proba, cohorts)
